...
COLNAMES = c("Region", 'Square Miles', 'Beneficiary Population', 'Population Density', 'Urban Reserves', 'White (%)', 'Black (%)', 'Median Income ($US)')
...
### Table 2 (EXPANDED). Demographic Comparison by Region. 
...
df2 <- read.csv(file.path(datadir, 'regional-comparison.csv'))[-1] %>%
  ...
  mutate(id = if_else(region == "Lowcountry", 1, 
         ...
         popden = round(popden * 0.386102), 0) %>%
  arrange(id, region, cat) %>%
  # mutate(urban = if_else(filter(region == 'Lowcountry') %>% is.na(urban), .$urban[2] + .$urban[3], urban)) %>%
  # mutate(medhhinc = paste("$", medhhinc, sep = '')) %>%
...
df2 %>%
  select(-region) %>%
  kable(format = "latex", booktabs = T, caption = NULL,
         col.names = c('Region', 'Square Miles', 'Beneficiary Population', 'Population Density', 'Urban Reserves', 'White (%)', 'Black (%)', 'Latinx (%)', 'Other (%)^a', 'People of Color (%)^b', 'Median Income ($US)')) %>%
  # kable_styling(latex_options = "scale_down", "striped",
  #               full_width = NULL) %>%
  pack_rows("Interstate", 1, 3) %>%
  pack_rows("South Carolina", 4, 6) %>%
