...
Area and Beneficiary Population is total for Lowcountry region, but mean for private and public beneficiary zones. Results are for the 10 mile beneficiary zone and 0.2 mile conservation reserve buffer zone pairing. One standard deviation is shown for means.
```{r include=TRUE, echo=FALSE}
...
df2 <- read.csv(file.path(datadir, 'regional-comparison.csv'))[-1] %>%
  mutate(id = if_else(region == "Lowcountry", 1, 
         ...
         sd_area = round(sd_sqkm * 0.386102,0),
         popden = round(popden * 0.386102), 0) %>%
  arrange(id, region, cat) %>%
  filter(region == 'Lowcountry') %>%
  mutate(area = paste0(area, " ± ", sd_area),
         tot_pop = paste0(tot_pop, " ± ", sd_tot_pop),
         popden = paste0(popden, " ± ", sd_popden),
         pwhite = paste0(pwhite, " ± ", sd_pwhite),
         pblack = paste0(pblack, " ± ", sd_pblack),
         pother = paste0(pother, " ± ", sd_pother),
         platinx = paste0(platinx, " ± ", sd_platinx),
         propPOC = paste0(propPOC, " ± ", sd_propPOC),
         medhhinc = paste0(medhhinc, " ± ", sd_medhhinc)) %>%
...
COLNAMES = c("Region", 'Area (mi²)', 'Beneficiary Population', 'Population Density (per mi²)', 'Urban Reserve Zones', 'White (%)', 'Black (%)', 'Median Income ($US)')
...
COLNAMES = c("Region", 'Area (mi²)', 'Beneficiary Population', 'Population Density (per mi²)', 'Reserve Zones (#)', 'White (%)', 'Black (%)', 'Median Income ($US)')
...
Area and Beneficiary Population is total for Lowcountry region, but mean for private and public beneficiary zones. Numbers shown are for the 10 mile beneficiary zone and 0.2 mile conservation reserve buffer zone pairing. One standard deviation is shown for means.
```{r include=TRUE, echo=FALSE}
...
df2 %>%
  select(-region) %>%
  kable(format = "latex", booktabs = T, caption = NULL,
         col.names = c('Region', 'Area (mi²)', 'Beneficiary Population', 'Population Density (per mi²)', 'Urban Reserve Zones', 'White (%)', 'Black (%)', 'Latinx (%)', 'Other (%)^a', 'People of Color (%)^b', 'Median Income ($US)')) %>%
  # kable_styling(latex_options = "scale_down", "striped",
  #               full_width = NULL) %>%
  pack_rows("Interstate", 1, 3) %>%
  pack_rows("South Carolina", 4, 6) %>%
